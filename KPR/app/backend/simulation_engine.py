...
INITIAL_ZONES_CONFIG = {
    "Z01": {"zone_name": "North Chennai",      "flood_boost": 0.60},   # HIGH
    "Z02": {"zone_name": "Central Chennai",    "flood_boost": 0.42},   # MEDIUM
    "Z03": {"zone_name": "Adyar",              "flood_boost": 0.75},   # CRITICAL/HIGH
    "Z04": {"zone_name": "Velachery",          "flood_boost": 0.95},   # CRITICAL
    "Z05": {"zone_name": "South-West Chennai", "flood_boost": 0.22},   # LOW
}
...
PERUNGUDI_CONFIG = {
    ...
    "latitude": 12.962,
    "longitude": 80.244,
    "flood_boost": 0.85,   # Starts HIGH, may escalate to CRITICAL
}
...
def compute_severity_score(
    ...
    """
    Compute a deterministic 0–100 severity score from zone metrics.

    Weights:
      flood_level     — 35 %
      critical_patients — 20 %
      blocked_roads    — 15 %
      pop_at_risk_pct  — 15 %
      shelter_gap      — 15 %
    """
    # Flood level: 0m = 0, 2.5m = 100
    flood_score = min(100.0, (flood_level / 2.5) * 100)

    # Critical patients: 0 = 0, 40+ = 100
    crit_score = min(100.0, (critical_patients / 40) * 100)

    # Blocked roads: 0 = 0, 10+ = 100
    road_score = min(100.0, (blocked_roads / 10) * 100)

    # Population at risk as proportion: 0% = 0, 35%+ = 100
    risk_pct = (population_at_risk / max(1, population)) * 100
    risk_score = min(100.0, (risk_pct / 35) * 100)

    # Shelter gap: 0% = 0, 100% deficit = 100
    if shelter_demand > 0 and shelter_available > 0:
        gap_pct = max(0, (shelter_demand - shelter_available) / shelter_demand)
    else:
        gap_pct = 0.3  # default moderate
    shelter_score = min(100.0, gap_pct * 100)
...
def severity_from_score(score: float):
    """Map severity score to Priority enum and label."""
...
class SimulationEngine:
    ...
    def _build_zone(self, zone_id: str, flood_boost: float, is_new: bool = False,
        ...
        severity_score = compute_severity_score(
            ...
            shelter_available=self.state.resources.shelter_available if self.state.resources else 0,
        )
        priority = severity_from_score(severity_score)

        score_label = f"Score {severity_score}/100"
        if priority == Priority.CRITICAL:
            priority_reason = f"{score_label} — Flood {flood_level}m, {critical_patients} critical patients, {blocked_count} blocked roads"
        elif priority == Priority.HIGH:
            priority_reason = f"{score_label} — Flood {flood_level}m, {critical_patients} critical patients"
        elif priority == Priority.MEDIUM:
            priority_reason = f"{score_label} — Moderate flood {flood_level}m, monitoring"
        else:
            priority_reason = f"{score_label} — Low flood {flood_level}m, situation stable"
    ...
    def _recalculate_zone_priority(self, zone: AffectedZone):
        score = compute_severity_score(
            flood_level=zone.flood_level,
            critical_patients=zone.critical_patients,
            blocked_roads=zone.blocked_roads,
            population=zone.population,
            population_at_risk=zone.population_at_risk,
            shelter_demand=zone.shelter_demand,
            shelter_available=self.state.resources.shelter_available if self.state.resources else 0,
        )
        zone.priority = severity_from_score(score)
        zone.priority_reason = f"Score {score}/100 — Flood {zone.flood_level}m, {zone.critical_patients} critical"
